...
class Dreamer_agent(nn.Module):
    ...
    def __init__(self, conf,step):
        ...
        if self.wm_type=='v2':
            self.wm = WorldModel_v2(conf,step)
    ...
    def training_step(self,
        ...
        if self.wm_type=='v2':
            ...
            loss_model,features, post, context, metrics,tensors=self.training_step(data,do_image_pred=do_image_pred)
    ...
    def dream(self, in_state: StateB, imag_horizon: int, dynamics_gradients=False,perturb='None'):
        features = []
        actions = []
        state = in_state
        self.wm.requires_grad_(False)  # Prevent dynamics gradiens from affecting world model
